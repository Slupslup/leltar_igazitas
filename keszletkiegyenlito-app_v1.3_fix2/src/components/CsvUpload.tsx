...
const CsvUpload: React.FC = () => {
  const { isLoading, selectedMonth, setLoading, setError, fetchStockData, warehouses, fetchProducts, products } = useStore();
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      setSelectedFiles(limitedFiles.map(file => ({ file, assignedWarehouse: '' })));
      setError(null); // Clear previous errors
      if (files.length > warehouses.length) {
        setError(`Figyelem: Több fájlt választott ki, mint ahány raktár (${warehouses.length} db) van. Csak az első ${warehouses.length} fájl lesz figyelembe véve.`);
      }
    }
  };

  const handleWarehouseAssignment = (index: number, warehouse: Warehouse | '') => {
  ...
  const assignedWarehouses = useMemo(() => {
    return new Set(selectedFiles.map(f => f.assignedWarehouse).filter(wh => wh !== ''));
  }, [selectedFiles]);
  ...
  const handleUpload = async () => {
    ...
    const unassignedFiles = selectedFiles.filter(f => f.assignedWarehouse === '');
    if (unassignedFiles.length > 0) {
      setError("Minden fájlhoz hozzá kell rendelni egy raktárat.");
      return;
    }
    if (assignedWarehouses.size !== warehouses.length) {
      setError("Minden raktárhoz pontosan egy fájlt kell hozzárendelni. Duplikált vagy hiányzó raktár hozzárendelés.");
      return;
    }
    ...
    try {
      ...
      if (currentProducts.length === 0) {
        await fetchProducts();
        currentProducts = useStore.getState().products; // Get updated products
      }
      const productMap = new Map(currentProducts.map(p => [p.name.trim().toLowerCase(), p.id]));
      const existingProductNamesLower = new Set(currentProducts.map(p => p.name.trim().toLowerCase()));
            ...
            complete: (results) => {
              try {
                ...
                for (let j = 2; j < rows.length; j++) {
                  ...
                  if (!productNameRaw || theoreticalStr === undefined || actualStr === undefined) continue;

                  const theoretical = parseInt(theoreticalStr.replace(/\s/g, ''), 10);
                  const actual = parseInt(actualStr.replace(/\s/g, ''), 10);

                  if (isNaN(theoretical) || isNaN(actual)) continue;
      ...
      if (allNewProductNames.size > 0) {
        ...
        if (insertError) {
          ...
          if (insertError.code === '23505') { // Unique violation
             console.warn('Product insertion conflict (23505), likely concurrent upload. Refetching products.');
             await fetchProducts(); // Refetch products to get IDs inserted by others
             finalProductMap = new Map(useStore.getState().products.map(p => [p.name.trim().toLowerCase(), p.id]));
          } else {
            throw new Error(`Hiba az új termékek beszúrásakor: ${insertError.message}`);
          }
        } else {
            // Update map with newly inserted products
            insertedProducts?.forEach(p => finalProductMap.set(p.name.trim().toLowerCase(), p.id));
        }
      }
      ...
      for (const warehouse in allParsedData) {
        ...
        warehouseItems.forEach(item => {
          const productId = finalProductMap.get(item.productName.trim().toLowerCase());
          if (productId) {
            ...
            console.warn(`Could not find product ID for '${item.productName}' after insertion/update.`);
            // Optionally: Decide whether to skip this item or throw an error
          }
        });
      }

      if (finalStockDataToInsert.length === 0 && Object.keys(allParsedData).length > 0) {
         throw new Error("Nem sikerült érvényes készletadatokat kinyerni a fájlokból a termékazonosítók hozzárendelése után.");
      }
      ...
      if (deleteError) {
        throw new Error(`Hiba a korábbi adatok törlésekor (${monthStart}): ${deleteError.message}`);
      }

      console.log(`Inserting ${finalStockDataToInsert.length} new stock records...`);
      if (finalStockDataToInsert.length > 0) {
        ...
        if (insertAllError) {
          ...
          throw new Error(`Hiba az új készletadatok beszúrásakor: ${insertAllError.message}. Az adatbázis inkonzisztens állapotba kerülhetett a(z) ${monthStart} hónapra.`);
        }
      }
  ...
  return (
    <div className="space-y-4">
      <div>
        <label htmlFor="csv-upload" className="block mb-1 text-sm font-medium text-gray-900">CSV Fájlok kiválasztása:</label>
        <input
          ref={fileInputRef}
          type="file"
          id="csv-upload"
        ...
        />
        <p className="mt-1 text-xs text-gray-500">Válassza ki az összes feltölteni kívánt CSV fájlt (max. {warehouses.length} db).</p>
      </div>

      {selectedFiles.length > 0 && (
        <div className="space-y-3 border border-gray-200 p-4 rounded-lg">
          <h3 className="text-md font-semibold text-gray-800">Fájlok hozzárendelése raktárakhoz:</h3>
          {selectedFiles.map((fileWithWh, index) => (
            <div key={index} className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4">
              <span className="text-sm font-medium text-gray-700 truncate flex-1" title={fileWithWh.file.name}>{fileWithWh.file.name}</span>
              <select
                value={fileWithWh.assignedWarehouse}
                onChange={(e) => handleWarehouseAssignment(index, e.target.value as Warehouse | '')}
              ...
              >
                ...
                {warehouses.map(wh => (
                  <option key={wh} value={wh} disabled={assignedWarehouses.has(wh) && fileWithWh.assignedWarehouse !== wh}>
                    {wh}
                  </option>
                ))}
              </select>
      ...
      <div className="flex justify-end">
        <button
          ...
          disabled={isLoading || selectedFiles.length === 0 || selectedFiles.length !== warehouses.length || selectedFiles.some(f => f.assignedWarehouse === '')}
        ...
        >
          {isLoading ? 'Feldolgozás...' : `Feltöltés és Feldolgozás (${warehouses.length} fájl)`}
        </button>
      </div>
    </div>
  );
