...
const TransferForm: React.FC = () => {
  const { products, warehouses, executeTransfer, isLoading, error, setError, fetchProducts } = useStore();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!fromWarehouse || !toWarehouse || !selectedProduct || quantity === '' || quantity <= 0) {
      setError("Minden mező kitöltése kötelező, és a mennyiségnek pozitívnak kell lennie.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
        {/* From Warehouse */}
        <div>
          <label htmlFor="from-warehouse" className="block mb-1 text-sm font-medium text-gray-900">Honnan:</label>
          <select 
            id="from-warehouse" 
            value={fromWarehouse}
        ...
        {/* To Warehouse */}
        <div>
          <label htmlFor="to-warehouse" className="block mb-1 text-sm font-medium text-gray-900">Hová:</label>
          <select 
            id="to-warehouse" 
            value={toWarehouse}
        ...
        <div className="lg:col-span-2">
          <label htmlFor="product-select" className="block mb-1 text-sm font-medium text-gray-900">Termék:</label>
          <select 
            id="product-select" 
        ...
        {/* Quantity */}
        <div>
          <label htmlFor="quantity" className="block mb-1 text-sm font-medium text-gray-900">Mennyiség:</label>
          <input 
            type="number" 
            id="quantity" 
            value={quantity}
