...
const calculateDifferenceAndHighlight = (
  ...
  const difference = theoretical - actual; // Correct calculation: actual - theoretical
...
export const useStore = create<AppState>((set, get) => ({
  ...
  fetchStockData: async (month) => {
    ...
    try {
      ...
      if (error) throw error;
      console.log(
        `Fetched ${snapshots?.length || 0} snapshots for ${monthStart}:`,
        snapshots
      ); // Log fetched snapshots
      ...
      for (const snap of snapshots || []) {
        ...
        if (productEntry) {
          ...
          if (WAREHOUSES.includes(warehouseKey)) {
            ...
            console.log(
              `Processing snapshot for product ${snap.product_id}, warehouse ${warehouseKey}`
            );
            ...
            console.warn(
              `Snapshot found for product ${snap.product_id} with invalid warehouse key: ${snap.warehouse}`
            );
          }
        } else {
          console.warn(
            `Snapshot found for unknown product ID: ${snap.product_id}`
          );
        }
      }
      console.log("Final processedData before setting state:", processedData); // Log final structure
